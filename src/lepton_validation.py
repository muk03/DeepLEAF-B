...
# Define options
options_1 = eos.Options(**{
    'form-factors': 'BSZ2015',  
    'model': 'WET',              
    'l': 'mu'
})

options_2 = eos.Options(**{
    'form-factors': 'BSZ2015',  
    'model': 'WET',              
    'l': 'tau'
})
...
param_wc_1_1 = params_1['cbmunumu::Re{cSR}']
param_wc_1_1.set(-5)
param_wc_1_2 = params_1['cbmunumu::Re{cVR}']
param_wc_1_2.set(0)

# Initialize the decay PDF object for the first set
dstarlnu_pdf_1 = eos.SignalPDF.make('B->D^*lnu::d^4Gamma', params_1, dstarlnu_kinematics, options_1)
...
param_wc_2_1 = params_2['cbtaunutau::Re{cSR}']
param_wc_2_1.set(-5)
param_wc_2_2 = params_2['cbtaunutau::Re{cVR}']
param_wc_2_2.set(0)

# Initialize the decay PDF object for the second set
dstarlnu_pdf_2 = eos.SignalPDF.make('B->D^*lnu::d^4Gamma', params_2, dstarlnu_kinematics, options_2)
...
csr_1 = params_1['cbmunumu::Re{cSR}'].evaluate()
# ['cbmunumu::Re{cVR}'].evaluate()
# ['cbtaunutau::Re{cSR}'].evaluate()
cvr_1 = params_1['cbmunumu::Re{cVR}'].evaluate()
# ['cbmunumu::Re{cVR}'].evaluate()
# ['cbtaunutau::Re{cVR}'].evaluate()
...
# Set up subplots
fig, axes = plt.subplots(2, 2, figsize=(20, 20), dpi=200)
...
for i, obs in enumerate(observables):
    ...
    #     where='post', color='cornflowerblue', linewidth=3, label=f'Tau Decay - cSR: {csr_1}, cVR: {cvr_1}'
    # )
    ax.bar(bin_edges_1[:-1], height_eos_1, width=np.diff(bin_edges_1), align='edge', 
           color='none', edgecolor='blue', hatch='\\\\', linewidth=2, 
           label=f'Muon Decay - cSR: {csr_1}, cVR: {cvr_1}')
    ...
    # ax.step(
    #     bin_edges_2, np.append(height_eos_2, height_eos_2[-1]),
    #     where='post', color='orange', linewidth=3, label=f'Muon Decay - cSR: {csr_2}, cVR: {cvr_2}'
    # )
    ...
    # ax.vlines(x_unq_2[0] - w/2, 0, height_eos_2[0], color='orange', linewidth=3)
    # ax.vlines(x_unq_2[-1] + w/2, 0, height_eos_2[-1], color='orange', linewidth=3)

    formatter = ticker.ScalarFormatter(useMathText=True)
    formatter.set_powerlimits((0.2, 1.5))  # Forces scientific notation with x10^4
    ax.yaxis.set_major_formatter(formatter)

    # ax.yaxis.offsetText.set_transform(ax.transAxes)

    # Now set the position: (x, y) where x around 1 means to the right edge and y controls vertical placement.
    # You can adjust these numbers to get the desired spacing.
    ax.yaxis.offsetText.set_position((0, 0.1))

    ax.set_xlabel(f'{lbl[i]}', fontsize=35)
    ax.set_ylabel('Integrated Bin Heights', fontsize=30)
    ax.set_title(f'{lbl[i]} Integrated Bin Heights')
    # ax.grid(True)

    ax.set_xlim((min(x_unq_1) - np.diff(x_unq_1)[-1], max(x_unq_2)) + np.diff(bin_edges_2)[-1]/2)
    # Add minor ticks
...
fig.legend(
    handles=handles[:2],  # Include the first three entries
    labels=labels[:2],    # Include the first three labels
    loc='lower center',   # Position the legend at the bottom center
    fontsize=35,
    ncol=2,               # Arrange legend entries in a single row
    bbox_to_anchor=(0.5, -0.05)  # Adjust position (x=center, y=slightly below the figure)
)

fig.suptitle('Integrated Observable Distributions \n Lepton Impact, Muon and Tau Decays', fontsize=35)

# Adjust layout and show the plot
plt.tight_layout()  # Leave space for the global legend and title
plt.show()
